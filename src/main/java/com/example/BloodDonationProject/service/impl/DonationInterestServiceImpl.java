...
public class DonationInterestServiceImpl implements DonationInterestService {
	...
	public DonationInterestResponseDTO expressInterest(DonationInterestRequestDTO requestDTO) {
		String donorId = requestDTO.getDonorId();
		...
		BloodRequest request = bloodRequestRepository.findById(UUID.fromString(requestId))
		...
		var existingInterest = donationInterestRepository.findByDonorIdAndRequestId(
				UUID.fromString(donorId), UUID.fromString(requestId));
		if (existingInterest.isPresent()) {
			throw new BadRequestException("Donor has already expressed interest in this request");
		}

		// Create new interest with status = PENDING and specified interest type
		DonationInterest interest = new DonationInterest(donor, request, requestDTO.getInterestType());
	 ...
	 */
	@Override
	public DonationInterestResponseDTO approveInterest(String interestId) {
		DonationInterest interest = donationInterestRepository.findById(UUID.fromString(interestId))
				.orElseThrow(() -> new ResourceNotFoundException("Interest not found with ID: " + interestId));

		if (!"PENDING".equals(interest.getStatus())) {
			throw new BadRequestException(
					"Only PENDING interests can be approved. Current status: " + interest.getStatus());
		}
	 ...
	 */
	@Override
	public DonationInterestResponseDTO declineInterest(String interestId) {
		DonationInterest interest = donationInterestRepository.findById(UUID.fromString(interestId))
				.orElseThrow(() -> new ResourceNotFoundException("Interest not found with ID: " + interestId));

		if (!"PENDING".equals(interest.getStatus())) {
			throw new BadRequestException(
					"Only PENDING interests can be declined. Current status: " + interest.getStatus());
		}
	...
	public List<DonationInterestResponseDTO> getInterestsForRequest(String requestId) {
		// Validate request exists
		bloodRequestRepository.findById(UUID.fromString(requestId))
				.orElseThrow(() -> new ResourceNotFoundException("Blood request not found with ID: " + requestId));

		List<DonationInterest> interests = donationInterestRepository.findByRequestId(UUID.fromString(requestId));
	...
	@Override
	@Transactional(readOnly = true)
	public List<DonationInterestResponseDTO> getInterestsForDonor(String donorId) {
		// Validate donor exists
		donorRepository.findById(UUID.fromString(donorId))
				.orElseThrow(() -> new ResourceNotFoundException("Donor not found with ID: " + donorId));

		List<DonationInterest> interests = donationInterestRepository.findByDonorId(UUID.fromString(donorId));
		return interests.stream()
				.map(this::mapToResponseDTO)
				.collect(Collectors.toList());
	}
	...
	@Override
	@Transactional(readOnly = true)
	public DonationInterestResponseDTO getInterestById(String interestId) {
		DonationInterest interest = donationInterestRepository.findById(UUID.fromString(interestId))
				.orElseThrow(() -> new ResourceNotFoundException("Interest not found with ID: " + interestId));
	...
	@Override
	@Transactional(readOnly = true)
	public List<DonationInterestResponseDTO> getInterestsByType(InterestType interestType) {
		List<DonationInterest> interests = donationInterestRepository.findByInterestType(interestType);
		return interests.stream()
				.map(this::mapToResponseDTO)
				.collect(Collectors.toList());
	}
	...
	@Override
	@Transactional(readOnly = true)
	public List<DonationInterestResponseDTO> getInterestsForRequestByType(String requestId, InterestType interestType) {
		// Validate request exists
		bloodRequestRepository.findById(UUID.fromString(requestId))
				.orElseThrow(() -> new ResourceNotFoundException("Blood request not found with ID: " + requestId));

		List<DonationInterest> interests = donationInterestRepository.findByRequestIdAndInterestType(
				UUID.fromString(requestId), interestType);
		return interests.stream()
				.map(this::mapToResponseDTO)
				.collect(Collectors.toList());
	}
	...
	@Override
	@Transactional(readOnly = true)
	public List<DonationInterestResponseDTO> getInterestsForDonorByType(String donorId, InterestType interestType) {
		// Validate donor exists
		donorRepository.findById(UUID.fromString(donorId))
				.orElseThrow(() -> new ResourceNotFoundException("Donor not found with ID: " + donorId));

		List<DonationInterest> interests = donationInterestRepository.findByDonorIdAndInterestType(
				UUID.fromString(donorId), interestType);
		return interests.stream()
				.map(this::mapToResponseDTO)
				.collect(Collectors.toList());
	}
